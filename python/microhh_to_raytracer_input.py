...
def solar_angles(lon, lat, day_of_year, year, seconds_since_midnight):
    ...
    doy_pi = 2.0 * np.pi * doy / days_per_year

    declination_angle = (
        0.006918
        - 0.399912 * np.cos(doy_pi)
        + 0.070257 * np.sin(doy_pi)
        - 0.006758 * np.cos(2 * doy_pi)
        + 0.000907 * np.sin(2 * doy_pi)
        - 0.002697 * np.cos(3 * doy_pi)
        + 0.00148 * np.sin(3 * doy_pi)
    )
    ...
    hour_solar_time = (
        (seconds_since_midnight / 3600) - a3 + radlon * (180.0 / np.pi / 15.0)
    )
    hour_angle = (hour_solar_time - 12) * 15.0 * (np.pi / 180)

    cos_zenith = np.sin(radlat) * np.sin(declination_angle) + np.cos(radlat) * np.cos(
        declination_angle
    ) * np.cos(hour_angle)
    ...
    cos_azimuth = (
        ...
        - np.sin(radlat) * np.cos(declination_angle) * np.cos(hour_angle)
    ) / cos_elevation

    azimuth = (
        np.arccos(cos_azimuth)
        if hour_angle <= 0.0
        else 2.0 * np.pi - np.arccos(cos_azimuth)
    )
...
def calc_sun_distance_factor(day_of_year, seconds_since_midnight):
    ...
    day_of_year = 228.3
    frac_doy = day_of_year + seconds_since_midnight / 86400.0
    t = 2.0 * np.pi * (frac_doy - 1.0) / 365.0
...
parser.add_argument(
    "-n", "--name", type=str, help="simulating name, that is, name of the .ini file"
)
...
parser.add_argument(
    ...
    "--path",
    type=str,
    help="Path to simulation files, defaults to current directory",
    default="./",
)
...
day_of_year = (
    int(
        (np.datetime64(dt_utc[:10]) - np.datetime64("{}-01-01".format(year)))
        / np.timedelta64(1, "D")
    )
    + 1
)
seconds_since_midnight = (
    int(dt_utc[11:13]) * 3600 + int(dt_utc[14:16]) * 60 + int(dt_utc[17:19]) + time
)
...
ftpnr_w = (4.0 / 3) * np.pi * 100e6 * 1e3
ftpnr_i = (4.0 / 3) * np.pi * 1e5 * 7e2
sig_fac = np.exp(np.log(1.34) * np.log(1.34))
rel = np.where(
    lwp > 0,
    1e6 * sig_fac * (lwp / dz[:, np.newaxis, np.newaxis] / ftpnr_w) ** (1.0 / 3),
    0,
)
rei = np.where(
    iwp > 0,
    1e6 * sig_fac * (iwp / dz[:, np.newaxis, np.newaxis] / ftpnr_i) ** (1.0 / 3),
    0,
)

rel = np.maximum(2.5, np.minimum(rel, 21.5))
rei = np.maximum(10.0, np.minimum(rei, 180.0))
...
grid_xh = np.append(
    grid.dim["xh"][:], grid.dim["xh"][-1] + (grid.dim["xh"][1] - grid.dim["xh"][0])
)
grid_yh = np.append(
    grid.dim["yh"][:], grid.dim["yh"][-1] + (grid.dim["yh"][1] - grid.dim["yh"][0])
)
...
nc_h2o[:] = np.append(
    h2o[:], np.tile(h2o_bg[zmin_idx:][:, None, None], (1, jtot, itot)), axis=0
)
nc_tlay = nc_out.createVariable("t_lay", "f8", ("lay", "y", "x"))
nc_tlay[:] = np.append(
    tlay[:], np.tile(tlay_bg[zmin_idx:][:, None, None], (1, jtot, itot)), axis=0
)
