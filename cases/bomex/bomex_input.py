...
for k in range(kmax):
    ...
    if z[k] <= 520.0:
        ...
        thl[k] = 298.7 + (z[k] - 520.0) * (302.4 - 298.7) / (1480.0 - 520.0)
    elif z[k] <= 2000:
        thl[k] = 302.4 + (z[k] - 1480.0) * (308.2 - 302.4) / (2000.0 - 1480.0)
    else:
        thl[k] = 308.2 + (z[k] - 2000.0) * (311.85 - 308.2) / (3000.0 - 2000.0)

    # specific humidity
    if z[k] <= 520.0:
        qt[k] = 1e-3 * (17.0 + z[k] * (16.3 - 17.0) / 520.0)
    elif z[k] <= 1480:
        qt[k] = 1.0e-3 * (16.3 + (z[k] - 520.0) * (10.7 - 16.3) / (1480.0 - 520.0))
    elif z[k] <= 2000:
        qt[k] = 1.0e-3 * (10.7 + (z[k] - 1480.0) * (4.2 - 10.7) / (2000.0 - 1480.0))
    else:
        qt[k] = 1.0e-3 * (4.2 + (z[k] - 2000.0) * (3.0 - 4.2) / (3000.0 - 2000.0))

    # u-wind component
    if z[k] <= 700.0:
        u[k] = -8.75
    else:
        u[k] = -8.75 + (z[k] - 700.0) * (-4.61 + 8.75) / (3000.0 - 700.0)

    # ugeo-wind component
    ugeo[k] = -10.0 + 1.8e-3 * z[k]

    # large scale vertical velocity
    if z[k] <= 1500:
        wls[k] = z[k] * (-0.65) / 1500.0
    elif z[k] <= 2100:
        wls[k] = -0.65 + (z[k] - 1500) * (0.65) / (2100.0 - 1500.0)

    # large scale temperature tendency
    if z[k] <= 1500:
        thlls[k] = -2.0
    else:
        thlls[k] = (-2.0) + (z[k] - 1500) * (2.0) / (3000.0 - 1500.0)

    # large scale moisture tendency
    if z[k] <= 300:
        qtls[k] = -1.2
    elif z[k] <= 500:
        qtls[k] = -1.2 + (z[k] - 300) * (1.2) / (500.0 - 300)

# normalize profiles to SI
# qtls /= 1000.  # from g/kg to kg/kg
wls /= 100.0  # from cm/s to m/s
thlls /= 86400.0  # from K/d to K/s
qtls *= 1.0e-8
