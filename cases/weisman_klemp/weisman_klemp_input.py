...
T_tr = 213.0  # Temperature  (K)
theta_tr = 343.0  # Potential temperature (K)
z_tr = 12000.0  # Height (m)

# Surface Parameters
theta_0 = 300.0  # Potential temperature (K)

# Shear Parameters
z_sh = 3000.0  # Heigth of maximum shear (m)

# Constants (from constants.h)
cp = 1005.0  # Specific heat of air at constant pressure [J kg-1 K-1]
...
Us = 25.0  # Shear velocity (m/s) [ 5 15 25 35 45 ]
...
def qv_rh(p, T, rh):
    return ep * esat_liq(T) * rh / (p - (1.0 - ep) * esat_liq(T) * rh)
...
for k in range(k_tr - 1, -2, -1):
    ...
    pfg = pow(pow(p_up, rdcp) + cpres / thl[k] / (1.0 + ep * qfg), cprd)
    ...
    p_up = pow(pow(p_up, rdcp) + cpres / thl[k] / (1.0 + ep * qfg), cprd)
...
nc_file = nc.Dataset(
    "weisman_klemp_input.nc", mode="w", datamodel="NETCDF4", clobber=True
)
