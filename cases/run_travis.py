...
dns_options = {"grid": {"itot": 8, "jtot": 8}, "time": {"endtime": 2, "savetime": 1}}

mpi_options = {"master": {"npx": 2, "npy": 2}}
...
for prec in precs:
    for mode in modes:
        ...
        for case in les_cases:
            err += mht.run_case(
                case, les_options, mpi_options, microhh_exec, mode, case, experiment
            )

        for case in dns_cases:
            err += mht.run_case(
                case, dns_options, mpi_options, microhh_exec, mode, case, experiment
            )

#
# Restart tests for a DNS + LES case
#
no_stats = {"stats": {"swstats": 0}, "cross": {"swcross": 0}, "column": {"swcolumn": 0}}

dns_perturbations = {
    "4th": {},
    "4th-4m": {
        "grid": {"swspatialorder": 4},
        "advec": {"swadvec": "4m"},
        "diff": {"swdiff": 4},
    },
    "2nd": {
