...
class Get_ref:
    def __init__(self, x, xh, z, zh, visc, time):
        ...
        for k in range(z.size):
            ...
            self.p[k, 0, :] = (
                0.25 * (cos(4.0 * pi * x) + cos(4.0 * pi * z[k])) - 0.25
            ) * (exp(-8.0 * pi**2.0 * visc * time) ** 2.0)
...
def plot(case_name, case_dir, experiment, float_type):
    ...
    # 2nd order data
    data16_2nd = Microhh(
        t, 16, 8, float_type, "{}/{}_itot016_swadvec2".format(case_dir, experiment)
    )
    data32_2nd = Microhh(
        t, 32, 16, float_type, "{}/{}_itot032_swadvec2".format(case_dir, experiment)
    )
    data64_2nd = Microhh(
        t, 64, 32, float_type, "{}/{}_itot064_swadvec2".format(case_dir, experiment)
    )
    data128_2nd = Microhh(
        t, 128, 64, float_type, "{}/{}_itot128_swadvec2".format(case_dir, experiment)
    )
    data256_2nd = Microhh(
        t, 256, 128, float_type, "{}/{}_itot256_swadvec2".format(case_dir, experiment)
    )

    ref16_2nd = Get_ref(
        data16_2nd.x, data16_2nd.xh, data16_2nd.z, data16_2nd.zh, visc, time
    )
    ref32_2nd = Get_ref(
        data32_2nd.x, data32_2nd.xh, data32_2nd.z, data32_2nd.zh, visc, time
    )
    ref64_2nd = Get_ref(
        data64_2nd.x, data64_2nd.xh, data64_2nd.z, data64_2nd.zh, visc, time
    )
    ref128_2nd = Get_ref(
        data128_2nd.x, data128_2nd.xh, data128_2nd.z, data128_2nd.zh, visc, time
    )
    ref256_2nd = Get_ref(
        data256_2nd.x, data256_2nd.xh, data256_2nd.z, data256_2nd.zh, visc, time
    )
    ...
    if t > 0:
        print(
            "convergence u_2nd",
            (log(errsu_2nd[-1]) - log(errsu_2nd[0])) / (log(dxs[-1]) - log(dxs[0])),
        )
        print(
            "convergence w_2nd",
            (log(errsw_2nd[-1]) - log(errsw_2nd[0])) / (log(dxs[-1]) - log(dxs[0])),
        )
    print(
        "convergence p_2nd",
    ...
    # 42 order data
    data16_4m = Microhh(
        t, 16, 8, float_type, "{}/{}_itot016_swadvec4m".format(case_dir, experiment)
    )
    data32_4m = Microhh(
        t, 32, 16, float_type, "{}/{}_itot032_swadvec4m".format(case_dir, experiment)
    )
    data64_4m = Microhh(
        t, 64, 32, float_type, "{}/{}_itot064_swadvec4m".format(case_dir, experiment)
    )
    data128_4m = Microhh(
        t, 128, 64, float_type, "{}/{}_itot128_swadvec4m".format(case_dir, experiment)
    )
    data256_4m = Microhh(
        t, 256, 128, float_type, "{}/{}_itot256_swadvec4m".format(case_dir, experiment)
    )

    ref16_4m = Get_ref(data16_4m.x, data16_4m.xh, data16_4m.z, data16_4m.zh, visc, time)
    ref32_4m = Get_ref(data32_4m.x, data32_4m.xh, data32_4m.z, data32_4m.zh, visc, time)
    ref64_4m = Get_ref(data64_4m.x, data64_4m.xh, data64_4m.z, data64_4m.zh, visc, time)
    ref128_4m = Get_ref(
        data128_4m.x, data128_4m.xh, data128_4m.z, data128_4m.zh, visc, time
    )
    ref256_4m = Get_ref(
        data256_4m.x, data256_4m.xh, data256_4m.z, data256_4m.zh, visc, time
    )
    ...
    # 4th order data
    data16_4th = Microhh(
        t, 16, 8, float_type, "{}/{}_itot016_swadvec4".format(case_dir, experiment)
    )
    data32_4th = Microhh(
        t, 32, 16, float_type, "{}/{}_itot032_swadvec4".format(case_dir, experiment)
    )
    data64_4th = Microhh(
        t, 64, 32, float_type, "{}/{}_itot064_swadvec4".format(case_dir, experiment)
    )
    data128_4th = Microhh(
        t, 128, 64, float_type, "{}/{}_itot128_swadvec4".format(case_dir, experiment)
    )
    data256_4th = Microhh(
        t, 256, 128, float_type, "{}/{}_itot256_swadvec4".format(case_dir, experiment)
    )

    ref16_4th = Get_ref(
        data16_4th.x, data16_4th.xh, data16_4th.z, data16_4th.zh, visc, time
    )
    ref32_4th = Get_ref(
        data32_4th.x, data32_4th.xh, data32_4th.z, data32_4th.zh, visc, time
    )
    ref64_4th = Get_ref(
        data64_4th.x, data64_4th.xh, data64_4th.z, data64_4th.zh, visc, time
    )
    ref128_4th = Get_ref(
        data128_4th.x, data128_4th.xh, data128_4th.z, data128_4th.zh, visc, time
    )
    ref256_4th = Get_ref(
        data256_4th.x, data256_4th.xh, data256_4th.z, data256_4th.zh, visc, time
    )
    ...
    if t > 0:
        print(
            "convergence u_4th",
            (log(errsu_4th[-1]) - log(errsu_4th[0])) / (log(dxs[-1]) - log(dxs[0])),
        )
        print(
            "convergence w_4th",
            (log(errsw_4th[-1]) - log(errsw_4th[0])) / (log(dxs[-1]) - log(dxs[0])),
        )
    print(
        "convergence p_4th",
    ...
    off2 = 0.01
    off4 = 0.002
    slope2 = off2 * (dxs[:] / dxs[0]) ** 2.0
    slope4 = off4 * (dxs[:] / dxs[0]) ** 4.0
    ...
    with PdfPages(file_name) as pdf:
        ...
        pcolormesh(
            data256_2nd.x,
            data256_2nd.z,
            data256_2nd.u[:, 0, :] - ref256_2nd.u[:, 0, :],
            rasterized=True,
        )
        ...
        pcolormesh(
            data256_4th.x,
            data256_4th.z,
            data256_4th.u[:, 0, :] - ref256_4th.u[:, 0, :],
            rasterized=True,
        )
        ...
        pcolormesh(
            data256_2nd.x,
            data256_2nd.z,
            data256_2nd.w[:, 0, :] - ref256_2nd.w[:, 0, :],
            rasterized=True,
        )
        xlim(min(data256_2nd.xh), max(data256_2nd.xh))
        ylim(min(data256_2nd.z), max(data256_2nd.z))
        ...
        pcolormesh(
            data256_4th.x,
            data256_4th.z,
            data256_4th.w[:, 0, :] - ref256_4th.w[:, 0, :],
            rasterized=True,
        )
        ...
        pcolormesh(
            data256_2nd.x,
            data256_2nd.z,
            data256_2nd.p[:, 0, :] - ref256_2nd.p[:, 0, :],
            rasterized=True,
        )
        ...
        pcolormesh(
            data256_4th.x,
            data256_4th.z,
            data256_4th.p[:, 0, :] - ref256_4th.p[:, 0, :],
            rasterized=True,
        )
        xlim(min(data256_4th.x), max(data256_4th.x))
        ylim(min(data256_4th.z), max(data256_4th.z))
