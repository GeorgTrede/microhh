...
if True:
    ...
    pl.subplot(121)
    pl.title("Fig 6a")
    pl.plot(c.u_flux / c.ustar**2.0, zhp, label="total")
    pl.plot(c.u_w / c.ustar**2.0, zhp, label="resolved")
    pl.plot((c.u_flux - c.u_w) / c.ustar**2.0, zhp, label="sgs")
    ...
    pl.subplot(122)
    pl.title("Fig 6b")
    pl.plot(c.v_flux / c.ustar**2.0, zhp, label="total")
    pl.plot(c.v_w / c.ustar**2.0, zhp, label="resolved")
    pl.plot((c.v_flux - c.v_w) / c.ustar**2.0, zhp, label="sgs")
    pl.legend(frameon=False)
    pl.xlabel("vw/u*^2")
    pl.ylabel("z*fc/u*")
...
if True:
    kappa = 0.4
    phi = kappa * c.zh / c.ustar * ((c.u_grad) ** 2 + (c.v_grad) ** 2) ** 0.5
