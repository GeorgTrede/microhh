...
for k in range(kmax):
    ...
    if z[k] < 740.0:
        ...
        thl[k] = 297.9 + (317.0 - 297.9) / (4000.0 - 740.0) * (z[k] - 740.0)

    if case == "gcss":
        if z[k] < 740.0:
            qt[k] = 16.0 + (13.8 - 16.0) / 740.0 * z[k]
        elif z[k] < 3260.0:
            qt[k] = 13.8 + (2.4 - 13.8) / (3260.0 - 740.0) * (z[k] - 740.0)
        else:
            qt[k] = 2.4 + (1.8 - 2.4) / (4000.0 - 3260.0) * (z[k] - 3260.0)

    elif case == "ss08":
        if z[k] < 740.0:
            qt[k] = 16.0 + (13.8 - 16.0) / 740.0 * z[k]
        elif z[k] < 3260.0:
            qt[k] = 13.8 + (4.4 - 13.8) / (3260.0 - 740.0) * (z[k] - 740.0)
        else:
            qt[k] = 4.4 + (3.6 - 4.4) / (4000.0 - 3260.0) * (z[k] - 3260.0)

    elif case == "test":
        q0 = 18.0
        q1 = 15.8
        if z[k] < 740.0:
            qt[k] = q0 + (q1 - q0) / 740.0 * z[k]
        elif z[k] < 3260.0:
            qt[k] = q1 + (2.4 - q1) / (3260.0 - 740.0) * (z[k] - 740.0)
        else:
            qt[k] = 2.4 + (1.8 - 2.4) / (4000.0 - 3260.0) * (z[k] - 3260.0)

    # Subsidence
    if z[k] < 2260:
        wls[k] = -0.005 * (z[k] / 2260.0)
    else:
        wls[k] = -0.005
    ...
    if z[k] < 2980:
        qtls[k] = -1.0 / 86400.0 + (1.3456 / 86400.0) * z[k] / 2980.0
    else:
        qtls[k] = 4e-6
...
def qsat(p, T):
    return ep * esat(T) / (p - (1.0 - ep) * esat(T))


ps = 101540.0
SST = 299.8
ths = SST / (ps / 1.0e5) ** (287.04 / 1005.0)
qs = qsat(ps, SST)
