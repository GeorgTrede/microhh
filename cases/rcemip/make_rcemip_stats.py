...
time_var[:] = nc_file.variables["time"][:] / 86400.0
...
add_0d_variable(
    pr_avg, "pr_avg", "domain avg. surface precipitation rate", "kg m-2 s-1"
)
del pr_avg

hfls_avg = (
    nc_file.groups["default"].variables["qt_flux"][:, 0]
    * nc_file.groups["thermo"].variables["rhoh"][:, 0]
    * Lv
)
add_0d_variable(
    hfls_avg, "hfls_avg", "domain avg. surface upward latent heat flux", "W m-2"
)

hfss_avg = (
    nc_file.groups["default"].variables["thl_flux"][:, 0]
    * nc_file.groups["thermo"].variables["rhoh"][:, 0]
    * cp
)
add_0d_variable(
    hfss_avg, "hfss_avg", "domain avg. surface upward sensible heat flux", "W m-2"
)
...
add_0d_variable(
    clwvi_avg, "clwvi_avg", "domain avg. water condensed water path", "kg m-2"
)
...
add_0d_variable(
    sprw_avg, "sprw_avg", "domain avg. saturated water vapor path", "kg m-2"
)
del sprw_avg
...
add_0d_variable(
    rlds_avg, "rlds_avg", "domain avg. surface downwelling longwave flux", "W m-2"
)
add_0d_variable(
    rlus_avg, "rlus_avg", "domain avg. surface upwelling longwave flux", "W m-2"
)
add_0d_variable(
    rsds_avg, "rsds_avg", "domain avg. surface downwelling shortwave flux", "W m-2"
)
add_0d_variable(
    rsus_avg, "rsus_avg", "domain avg. surface upwelling shortwave flux", "W m-2"
)
...
add_0d_variable(
    rldscs_avg,
    "rldscs_avg",
    "domain avg. surface downwelling longwave flux - clear sky",
    "W m-2",
)
add_0d_variable(
    rluscs_avg,
    "rluscs_avg",
    "domain avg. surface upwelling longwave flux - clear sky",
    "W m-2",
)
add_0d_variable(
    rsdscs_avg,
    "rsdscs_avg",
    "domain avg. surface downwelling shortwave flux - clear sky",
    "W m-2",
)
add_0d_variable(
    rsuscs_avg,
    "rsuscs_avg",
    "domain avg. surface upwelling shortwave flux - clear sky",
    "W m-2",
)
...
add_0d_variable(
    rlut_avg, "rlut_avg", "domain avg. TOA upwelling longwave flux", "W m-2"
)
add_0d_variable(
    rsdt_avg, "rsdt_avg", "domain avg. TOA downwelling shortwave flux", "W m-2"
)
add_0d_variable(
    rsut_avg, "rsut_avg", "domain avg. TOA upwelling shortwave flux", "W m-2"
)
...
add_0d_variable(
    rlutcs_avg,
    "rlutcs_avg",
    "domain avg. TOA upwelling longwave flux - clear sky",
    "W m-2",
)
add_0d_variable(
    rsutcs_avg,
    "rsutcs_avg",
    "domain avg. TOA upwelling shortwave flux - clear sky",
    "W m-2",
)
...
time_var[:] = nc_file.variables["time"][:] / 86400.0  # Convert to days.
...
add_1d_variable(
    phydro_avg, "phydro", "domain avg. air hydrostatic pressure profile", "Pa", "z"
)
del phydro_avg
...
add_1d_variable(
    phydroh_avg,
    "phydroh",
    "domain avg. air hydrostatic pressure profile at flux level",
    "Pa",
    "zh",
)
del phydroh_avg
...
hur_avg = nc_file.groups["thermo"].variables["rh"][:, :] * 100.0
add_1d_variable(
    hus_avg, "hus_avg", "domain avg. specific humidity profile", "kg kg-1", "z"
)
...
pli_avg = (
    nc_file.groups["default"].variables["qs"][:, :]
    + nc_file.groups["default"].variables["qg"][:, :]
)
add_1d_variable(
    clw_avg,
    "clw_avg",
    "domain avg. mass fraction of the cloud liquid water profile",
    "kg kg-1",
    "z",
)
add_1d_variable(
    cli_avg,
    "cli_avg",
    "domain avg. mass fraction of the cloud ice profile",
    "kg kg-1",
    "z",
)
add_1d_variable(
    plw_avg,
    "plw_avg",
    "domain avg. mass fraction of the precipitating liquid water profile",
    "kg kg-1",
    "z",
)
add_1d_variable(
    pli_avg,
    "pli_avg",
    "domain avg. mass fraction of the precipitating ice profile",
    "kg kg-1",
    "z",
)
...
qv_avg = (
    nc_file.groups["default"].variables["qt"][:, :]
    - nc_file.groups["thermo"].variables["ql"][:, :]
    - nc_file.groups["thermo"].variables["qi"][:, :]
)
thetae_avg = (nc_file.groups["thermo"].variables["T"][:, :] + Lv / cp * qv_avg) / exner
add_1d_variable(
    theta_avg, "theta_avg", "domain avg. potential temperature profile", "K", "z"
)
add_1d_variable(
    thetae_avg,
    "thetae_avg",
    "domain avg. equivalent potential temperature profile",
    "K",
    "z",
)
del (theta_avg, thetae_avg, exner, qv_avg)

cldfrac_avg = np.maximum(
    nc_file.groups["thermo"].variables["ql_frac"][:, :],
    nc_file.groups["thermo"].variables["qi_frac"][:, :],
)
...
fac = -1.0 / (nc_file.groups["thermo"].variables["rho"][:, :] * cp * dz[None, :])
sw_heating_rate = fac * (
    sw_flux_up[:, 1:] - sw_flux_up[:, :-1] - sw_flux_dn[:, 1:] + sw_flux_dn[:, :-1]
)
lw_heating_rate = fac * (
    lw_flux_up[:, 1:] - lw_flux_up[:, :-1] - lw_flux_dn[:, 1:] + lw_flux_dn[:, :-1]
)
sw_clear_heating_rate = fac * (
    sw_clear_flux_up[:, 1:]
    - sw_clear_flux_up[:, :-1]
    - sw_clear_flux_dn[:, 1:]
    + sw_clear_flux_dn[:, :-1]
)
lw_clear_heating_rate = fac * (
    lw_clear_flux_up[:, 1:]
    - lw_clear_flux_up[:, :-1]
    - lw_clear_flux_dn[:, 1:]
    + lw_clear_flux_dn[:, :-1]
...
add_1d_variable(
    sw_heating_rate,
    "tntrs_avg",
    "domain avg. shortwave radiative heating rate profile",
    "K s-1",
    "z",
)
add_1d_variable(
    lw_heating_rate,
    "tntrl_avg",
    "domain avg. longwave radiative heating rate profile",
    "K s-1",
    "z",
)
add_1d_variable(
    sw_clear_heating_rate,
    "tntrscs_avg",
    "domain avg. shortwave radiative heating rate profile",
    "K s-1",
    "z",
...
nc_file_out = "microhh_{0}_2d_{1}.nc".format(case_name, var_out)
subprocess.run(
    "ncks -O -h -d zh,0,0 {0} {1}".format(nc_file_in, nc_file_out), shell=True
)
nc_2d = nc.Dataset(nc_file_out, "r+")
nc_2d.renameVariable(var_in, var_out)
nc_2d_var = nc_2d.variables[var_out]
...
nc_file_out = "microhh_{0}_2d_{1}.nc".format(case_name, var_out)
subprocess.run(
    "ncks -O -h -d zh,1,1 {0} {1}".format(nc_file_in, nc_file_out), shell=True
)
nc_2d = nc.Dataset(nc_file_out, "r+")
nc_2d.renameVariable(var_in, var_out)
nc_2d_var = nc_2d.variables[var_out]
...
nc_file_out = "microhh_{0}_2d_{1}.nc".format(case_name, var_out)
subprocess.run(
    "ncks -O -h -d z,0,0 {0} {1}".format(nc_file_in, nc_file_out), shell=True
)
nc_2d = nc.Dataset(nc_file_out, "r+")
nc_2d.renameVariable(var_in, var_out)
nc_2d_var = nc_2d.variables[var_out]
