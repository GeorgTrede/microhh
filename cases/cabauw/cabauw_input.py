...
def create_case_input(
    ...
    if use_htessel:
        copy_or_link(
            "../../misc/van_genuchten_parameters.nc", "van_genuchten_parameters.nc"
        )
    if use_rrtmgp:
        if gpt_set == "256_224":
            copy_or_link(
                "../../rte-rrtmgp-cpp/rrtmgp-data/rrtmgp-gas-lw-g256.nc",
                "coefficients_lw.nc",
            )
            copy_or_link(
                "../../rte-rrtmgp-cpp/rrtmgp-data/rrtmgp-gas-sw-g224.nc",
                "coefficients_sw.nc",
            )
        elif gpt_set == "128_112":
            copy_or_link(
                "../../rte-rrtmgp-cpp/rrtmgp-data/rrtmgp-gas-lw-g128.nc",
                "coefficients_lw.nc",
            )
            copy_or_link(
                "../../rte-rrtmgp-cpp/rrtmgp-data/rrtmgp-gas-sw-g112.nc",
                "coefficients_sw.nc",
            )
        else:
        ...
        copy_or_link(
            "../../rte-rrtmgp-cpp/rrtmgp-data/rrtmgp-clouds-lw.nc",
            "cloud_coefficients_lw.nc",
        )
        copy_or_link(
            "../../rte-rrtmgp-cpp/rrtmgp-data/rrtmgp-clouds-sw.nc",
    ...
    if not use_homogeneous_ls:
        ...
        lsm_data = LSM_input(
            itot, jtot, ktot=4, TF=TF, debug=True, exclude_fields=exclude
        )
        ...
        lsm_data.c_veg[~mask] = ini["land_surface"]["c_veg"] / 3.0

        lsm_data.lai[mask] = ini["land_surface"]["lai"]
        lsm_data.lai[~mask] = ini["land_surface"]["lai"] / 2.0
...
if __name__ == "__main__":
    ...
    use_homogeneous_ls = (
        True  # False = checkerboard pattern (some...) land-surface fields.
    )
