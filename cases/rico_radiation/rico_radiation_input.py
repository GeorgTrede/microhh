...
# Create input file
nc_file = nc.Dataset(
    "rico_radiation_input.nc", mode="w", datamodel="NETCDF4", clobber=True
)
...
if use_rt and not diurnal_cycle:
    raise Exception(
        "Unfortunately, we do not support fixed azmiuth angles yet, hence ray tracing with fixed solar angles is not possible"
    )
