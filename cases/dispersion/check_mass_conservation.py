...
stats = xr_read_all(
    "dispersion.default.0000000.nc", groups=["default", "thermo"], decode_times=False
)
...
for time in range(0, ini["time"]["endtime"] + 1, ini["time"]["savetime"]):
    ...
    for i, scalar in enumerate(scalars):
        ...
        print(
            'Time = {0:5d}, scalar "{1}": correct mass = {2:8.2f} kg, integral field = {3:8.2f} kg'.format(
                time, scalar, correct_mass[i], mass
            )
        )
