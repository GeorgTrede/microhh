...
if not no_frames:
    ...
    def process_frame(i, co2_slice, u_slice, w_slice, z_level, z, uflux, co2_max, u_min, u_max, w_min, w_max):
        ...
        plt.subplot(3, 1, 1)
        plt.imshow(co2_slice, vmin=0, vmax=co2_max*0.8, extent=[0, 12.8, 0, 5.0])
        plt.title("CO2 (integrated)")
        plt.xlabel("x [km]")
        plt.ylabel("y [km]")
        plt.colorbar(shrink=0.7)
        plt.subplot(3, 1, 2)
        plt.imshow(u_slice, vmin=u_min, vmax=u_max, extent=[0, 12.8, 0, 5.0])
        plt.title(f"u (at z={int(z[z_level])}m, in m/s)")
        plt.xlabel("x [km]")
        plt.ylabel("y [km]")
        plt.colorbar(shrink=0.7)
        plt.subplot(3, 1, 3)
        plt.imshow(w_slice, vmin=w_min, vmax=w_max, extent=[0, 12.8, 0, 5.0])
