...
parser.add_argument(
    "-m", "--modes", nargs="*", help="mode of the cross section", choices=cross_modes
)
...
parser.add_argument(
    "-c", "--nocompression", help="do not compress the netcdf file", action="store_true"
)
...
starttime = (
    float(args.starttime) if args.starttime is not None else nl["time"]["starttime"]
)
endtime = float(args.endtime) if args.endtime is not None else nl["time"]["endtime"]
sampletime = (
    float(args.sampletime) if args.sampletime is not None else nl["cross"]["sampletime"]
)
