...
class Data:
    def __init__(self, path, nx, nz, N2):
        ...
        startzh = 0
        self.u_ref = u_hires[
            startz : nz_hires - 1 : stepz, startxh : nx_hires - 1 : stepx
        ]
        self.w_ref = w_hires[
            startzh : nz_hires - 1 : stepz, startx : nx_hires - 1 : stepx
        ]
        self.b_ref = b_hires[
            startz : nz_hires - 1 : stepz, startx : nx_hires - 1 : stepx
        ]
        ...
        for k in range(self.z.size):
            self.u_error += np.sqrt(
                self.dx * sum((self.u[k, :] - self.u_ref[k, :]) ** 2)
            )
            self.w_error += np.sqrt(
                self.dx * sum((self.w[k, :] - self.w_ref[k, :]) ** 2)
            )
            self.b_error += np.sqrt(
                self.dx * sum((self.b[k, :] - self.b_ref[k, :]) ** 2)
            )
...
print(
    "convergence u",
    (np.log(data128.u_error) - np.log(data32.u_error))
    / (np.log(data128.dx) - np.log(data32.dx)),
)
print(
    "convergence w",
    (np.log(data128.w_error) - np.log(data32.w_error))
    / (np.log(data128.dx) - np.log(data32.dx)),
)
print(
    "convergence p",
    (np.log(data128.b_error) - np.log(data32.b_error))
    / (np.log(data128.dx) - np.log(data32.dx)),
)
