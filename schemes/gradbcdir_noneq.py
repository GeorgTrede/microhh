...
def geterror(u, uref, n=2):
    error = sqrt(sum((uref - u) ** 2.0) / u.size)
    # error = abs(max(uref - u))
    return error


def refdata(n):
    a = linspace(1.0 / (n + 1.0), 1.0 - 1.0 / (n + 1.0), n)
    # x = 0.5 * (1. - stretch**a) / (1. - stretch)
    x = 0.25 + 0.25 * tanh(stretch * (2.0 * a - 1.0)) / tanh(stretch)
    u = sin(2.0 * pi * x / xsize)
    return x, u
...
def gx2nd(x, u):
    ...
    xcalc[iend] = 2.0 * xsize - xcalc[iend - 1]
    ...
    guref = 2.0 * pi / xsize * cos(2.0 * pi * x / xsize)
...
def gx4th(x, u):
    ...
    # ghost cell
    xcalc[istart - 1] = -2.0 * xcalc[istart] + (1.0 / 3.0) * xcalc[istart + 1]
    xcalc[istart - 2] = -9.0 * xcalc[istart] + 2.0 * xcalc[istart + 1]
    # xcalc[istart-1] = -xcalc[istart  ]
    # xcalc[istart-2] = -xcalc[istart+1]

    xcalc[iend] = (
        (8.0 / 3.0) * xsize - 2.0 * xcalc[iend - 1] + (1.0 / 3.0) * xcalc[iend - 2]
    )
    xcalc[iend + 1] = 8.0 * xsize - 9.0 * xcalc[iend - 1] + 2.0 * xcalc[iend - 2]
    # xcalc[iend  ] = 2.*xsize - xcalc[iend-1]
    # xcalc[iend+1] = 2.*xsize - xcalc[iend-2]

    ucalc[istart - 1] = -2.0 * ucalc[istart] + (1.0 / 3.0) * ucalc[istart + 1]
    ucalc[istart - 2] = -9.0 * ucalc[istart] + 2.0 * ucalc[istart + 1]

    ucalc[iend] = -2.0 * ucalc[iend - 1] + (1.0 / 3.0) * ucalc[iend - 2]
    ucalc[iend + 1] = -9.0 * ucalc[iend - 1] + 2.0 * ucalc[iend - 2]
...
print(
    "convergence 2nd",
    (log(errs_2nd[-1]) - log(errs_2nd[0])) / (log(dxs[-1]) - log(dxs[0])),
)
print(
    "convergence 4th",
    (log(errs_4th[-1]) - log(errs_4th[0])) / (log(dxs[-1]) - log(dxs[0])),
)
