...
def refdata(n):
    ...
    xi = linspace(0.0, xsize, n + 1)[0:n]
    u = cos(2.0 * pi * x / xsize)
    return x, xi, u
...
def ix2nd(x, u):
    ...
    for i in range(istart, iend):
        iu[i - igc] = (ucalc[i] + ucalc[i - 1]) / 2.0

    iuref = cos(2.0 * pi * x / xsize)
...
def ix4th(x, u):
    ...
    # ghost cell
    ucalc[istart - 1] = (
        21.0 * ucalc[istart] + 3.0 * ucalc[istart + 1] - ucalc[istart + 2]
    ) / 23.0
    ucalc[iend] = (
        21.0 * ucalc[iend - 1] + 3.0 * ucalc[iend - 2] - ucalc[iend - 3]
    ) / 23.0

    i = istart
    iu[i - igc] = (
        5.0 * ucalc[i - 1] + 15.0 * ucalc[i] - 5.0 * ucalc[i + 1] + ucalc[i + 2]
    ) / 16.0
    for i in range(istart + 1, iend):
        iu[i - igc] = (
            -ucalc[i - 2] + 9.0 * ucalc[i - 1] + 9.0 * ucalc[i] - ucalc[i + 1]
        ) / 16.0
