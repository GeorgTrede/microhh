...
def laplx2nd(x, u):
    ...
    for i in range(istart, iend):
        laplu[i - igc] = (ucalc[i - 1] - 2.0 * ucalc[i] + ucalc[i + 1]) / (dx**2.0)

    lapluref = -4 * pi**2.0 / xsize**2.0 * cos(2.0 * pi * x / xsize)
...
def laplx4th(x, u):
    ...
    ucalc[istart - 1] = (
        1360.0 * ucalc[istart]
        + 720.0 * ucalc[istart + 1]
        - 400.0 * ucalc[istart + 2]
        + 80.0 * ucalc[istart + 3]
    ) / 1760.0
    ucalc[iend] = (
        1360.0 * ucalc[iend - 1]
    ...
    for i in range(istart + 1, iend - 1):
        laplu[i - igc] = (
            -1.0 * (ucalc[i - 2] + ucalc[i + 2])
            + 16.0 * (ucalc[i - 1] + ucalc[i + 1])
            - 30.0 * ucalc[i]
        ) / (12.0 * dx**2.0)
    i = iend - 1
    laplu[i - igc] = (
